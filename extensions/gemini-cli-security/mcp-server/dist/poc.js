...
export async function runPoc({ filePath, }, dependencies = { fs, path, execAsync, execFileAsync }) {
    try {
        ...
        if (ext === '.py') {
            ...
            const checkExists = async (p) => dependencies.fs.access(p).then(() => true).catch(() => false);
        ...
        try {
            output = (await dependencies.execFileAsync(runCmd, runArgs, execOptions));
        }
        catch (error) {
