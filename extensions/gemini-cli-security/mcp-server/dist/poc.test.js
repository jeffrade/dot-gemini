...
describe('runPoc', () => {
    ...
    it('should execute a Node.js file', async () => {
        ...
        const result = await runPoc({ filePath: `${POC_DIR}/test.js` }, {
            fs: {
    ...
    it('should execute a Python file', async () => {
        const mockExecAsync = vi.fn(async () => { return { stdout: '', stderr: '' }; });
        const mockExecFileAsync = vi.fn(async () => { return { stdout: 'output', stderr: '' }; });
        const result = await runPoc({ filePath: `${POC_DIR}/test.py` }, { fs: { access: vi.fn().mockRejectedValue(new Error()), writeFile: vi.fn() }, path: mockPath, execAsync: mockExecAsync, execFileAsync: mockExecFileAsync });
    ...
    it('should execute a Go file', async () => {
        const mockExecAsync = vi.fn(async () => { return { stdout: '', stderr: '' }; });
        const mockExecFileAsync = vi.fn(async () => { return { stdout: 'output', stderr: '' }; });
        const result = await runPoc({ filePath: `${POC_DIR}/test.go` }, { fs: { access: vi.fn().mockRejectedValue(new Error()), writeFile: vi.fn() }, path: mockPath, execAsync: mockExecAsync, execFileAsync: mockExecFileAsync });
    ...
    it('should handle execution errors', async () => {
        ...
        const mockExecFileAsync = vi.fn(async (file, args) => {
            throw new Error('Execution failed');
        });
        const result = await runPoc({ filePath: `${POC_DIR}/error.js` }, {
    ...
    it('should fail when accessing file outside of allowed directory', async () => {
        ...
        const result = await runPoc({ filePath: '/tmp/malicious.js' }, { fs: { writeFile: vi.fn() }, path: mockPath, execAsync: mockExecAsync, execFileAsync: mockExecFileAsync });
    ...
    it('should cleanup path traversal temp file if it exists', async () => {
        ...
        await runPoc({ filePath: `${POC_DIR}/test.js` }, {
            fs: {
                access: mockAccess,
                unlink: mockUnlink,
