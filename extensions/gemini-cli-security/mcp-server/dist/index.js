...
import { SECURITY_DIR_NAME } from './constants.js';
...
server.tool('get_audit_scope', 'Gets the git diff of the current changes. Can optionally compare two specific branches.', {
...
server.tool('get_files_to_audit', 'Lists relevant files for auditing by filtering out irrelevant files and folders.', {}, (() => {
...
server.tool(RUN_POC_TOOL_NAME, RUN_POC_TOOL_DESCRIPTION, RunPocArgsSchema.shape, getRunPocMessages);
server.tool('convert_report_to_json', `Converts the Markdown security report into a JSON file named security_report.json in the ${SECURITY_DIR_NAME} folder.`, {}, (async () => {
    try {
...
server.registerTool(POC_CONTEXT_TOOL_NAME, {
    description: POC_CONTEXT_TOOL_DESCRIPTION,
    inputSchema: PocContextArgsSchema,
}, getPocContext);
...
server.tool(SECURITY_PATCH_CONTEXT_TOOL_NAME, SECURITY_PATCH_CONTEXT_TOOL_DESCRIPTION, SecurityPatchContextArgsSchema.shape, getSecurityPatchContextMessages);
server.tool('install_dependencies', 'Executes a script file inside workspace.', {
...
server.tool('get_line_count', 'Gets the total line count of a list of files.', {
    files: z
        .array(z.string())
