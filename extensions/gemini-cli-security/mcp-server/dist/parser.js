 ...
 */
const buildPattern = (label) => {
    const key = label.toLowerCase();
    if (patternCache.has(key))
        return patternCache.get(key);
    const escapedLabel = escapeRegExp(label);
    const rx = new RegExp(`(?:-?\\s*\\**)?${escapedLabel}\\**:\\s*([\\s\\S]*?)(?=\\n(?:-?\\s*\\**)?(?:${FIELD_NAMES})|$)`, 'i');
 ...
 */
function extractFromSection(section, label) {
 ...
 */
export function parseMarkdownToDict(content) {
    const findings = [];
    ...
    for (let section of sections) {
        ...
        if (!section || !section.includes("Vulnerability:"))
            continue;
